...
int i;
	...
	if (strcasecmp(argv[1], "set-integration-window") == 0) {
		send_usb(handle, COMMAND_SET_INTEGRATION_WINDOW, atoi(argv[2]));
	}
	else if (strcasecmp(argv[1], "get-integration-window") == 0) {
		send_usb(handle, COMMAND_GET_INTEGRATION_WINDOW, 0);
		printf("%d\n", get_usb(handle, buf));
	}
	else if (strcasecmp(argv[1], "get-seq-wnd") == 0) {
